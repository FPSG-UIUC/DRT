...
int main(int argc, char *argv[])
{
	...
	int i_top = atoi(argv[5]);
	int j_top = atoi(argv[6]);
	int k_top = atoi(argv[7]);
	...
	// Extract Top mbuild: {instant, serial, parallel}, Parallelism factor
 	Parameters * params= new Parameters(kernel::SpMM, CSX::CSF, CSX::Dense, CSX::Dense, dense_col_size,
			tiling::t_static, static_distributor::round_robin, arch::outerProdMiddle,
			i_top, j_top, k_top,
	...
	// 30MB LLB memory
	LLB_Mem * llb = new LLB_Mem(stats, params, 30*1024*1024, 0.01, 0.95, 0.4);
	...
	Scheduler_SpMM_7 * sched_7 = new Scheduler_SpMM_7(mat, params, stats, llb);
	sched_7->Run();
	if(stats->Get_cycles()){
...
*/
	...
	printf("runtime: 0, cycles: 0, busy_cycles: 0\n");
