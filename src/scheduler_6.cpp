#ifndef SCHEDULER_6_CPP
#define SCHEDULER_6_CPP

#include "scheduler_6.h"

Scheduler_6::Scheduler_6(Matrix * mtx, Parameters * params, Stats * stats, LLB_Mem * llb){
	this->matrix = mtx;
	this->params = params;
	this->stats = stats; 
	this->llb = llb;
...
void Scheduler_6::Schedule(int i_idx, int j_idx_start, int j_idx_stop, int k_idx, 
		...
		for(int j_inner = j_idx_start; j_inner < j_idx_stop; j_inner++){
			...
			if ((action_bytes) | (cycles_comp)){
				if(params->getIntersectModelDist() == intersect_dist::sequential){
					...
					starting_cycle = pe_time[round_robin_slot];
					...
					pe_time[round_robin_slot] +=  ((uint64_t)cycles_comp + excessCycles);
					...
					// Start and end cycle for currrent tile multiplication
					// Since we are going round-robin, this should not matter at all
					starting_cycle = *std::min_element(pe_time, pe_time+params->getPECount());
					...
					for(uint64_t i = (uint64_t)cycles_comp; i>0;/* */){
						...
						i-=cycles_curr_pe;
						pe_time[round_robin_slot] += cycles_curr_pe;
						uint64_t max_cycles = pe_time[round_robin_slot];
						round_robin_slot = (++round_robin_slot) % (params->getPECount());
...
uint64_t Scheduler_6::updateBWLogAndReturnExcess(uint64_t starting_cycle, 
...
//		the fetched row
void Scheduler_6::SyncPETimesWithMinCycle(uint64_t min_val){
...
void Scheduler_6::CalcOReuse(int k_idx, int j_idx_start, int & j_idx_stop){
	int a_reuse = params->getAReuse();
...
void Scheduler_6::PreCalculateAORowsSize(int j_start, int j_end, 
...
//   the smallest end cycle time and gives its space to new rows. The number of new rows
//   is returned in numRows (range [0,o_tiled_rows] ).
void Scheduler_6::FetchAORows(int & numRows,
...
void Scheduler_6::CalcBLLBHorizontalSum(int j_start, int j_end, 
		int k_start, int k_end, int * b_llb_horizontalSum){
...
uint64_t Scheduler_6::AccumulateSize_AwrtB(int i_idx, 
...
//  they should be loaded or not 
uint64_t Scheduler_6::AccumulateSize_OwrtAB(int i_idx,	
...
uint32_t Scheduler_6::ShouldIFetchThisOTile(int i_idx, 
...
uint64_t Scheduler_6::AccumulateSize(char mat_name, int d1_start, int d1_end,
