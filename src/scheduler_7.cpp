/* Written by Hadi Asghari-Moghaddam
 * University of Illinois at Urbana-Champaign @2020
 *
 * The final result correctnes was confirmed on
 * Sep. 2nd, 2020. In case you need a revert, pull today's commit
 * from the git repo
 */


#ifndef SCHEDULER_7_CPP
#define SCHEDULER_7_CPP
...
Scheduler_7::Scheduler_7(Matrix * mtx, Parameters * params, Stats * stats, LLB_Mem * llb){
	this->matrix = mtx;
	this->params = params;
	this->stats = stats;
	this->llb = llb;
...
int Scheduler_7::Run(){
	...
	for(int k_idx = 0; k_idx < o_tiled_cols; /* EMPTY */){
        printf("Debug: top k %d of %d\n", k_idx, o_tiled_cols); fflush(stdout);
...
void Scheduler_7::Schedule(int i_idx, int j_idx_start, int j_idx_stop, int k_idx){

	//printf("1: i_idx:%d, j_idx: %d-%d, k_idx: %d\n", i_idx, j_idx_start, j_idx_stop, k_idx);
	...
	uint64_t o_r_row_acc = 0, o_r_row_calc = vecCommittedRows_OSize[i_idx];
	...
	for(int k_inner = k_idx; k_inner < std::min(k_idx+a_reuse, o_tiled_cols); k_inner++){
		...
		// Size of the output tile before and after computation. number of intersections contributed
		//   to the current output tile
		OSize_preComp = 0; OSize_postComp = 0; o_tile_intersect_count = 0;
		OSize_preComp = matrix->getCOOSize('O', i_idx, k_inner);

		for(int t_idx = 0; t_idx<a_entries_count.size();t_idx++){
			...
			// o_tile_intersect_ouput shows how many intersection have been found so far
			...
			*middle_operator += (overhead_middle_extract + overhead_middle_intersect);
			...
			for (std::vector<int>::iterator it = intersect_vector.begin();
				...
				if ((action_bytes) | (cycles_comp) | (middle_DOT_traffic)){
					...
					// hmmm, we shoul bring data as soon as possible, not wait for the PE time to occur
		...
		if((o_tile_intersect_count == 0) & (OSize_postComp > OSize_preComp)){
			printf("There was no intersect but size chahnged! intersects: %d, pre: %lu, post %lu\n",
					o_tile_intersect_count, OSize_preComp, OSize_postComp);
		}
		if(o_tile_intersect_count > 0){
			OSize_postComp = matrix->getCOOSize('O', i_idx, k_inner);

			// Update the llb usage for all the tiles
			if(OSize_preComp > OSize_postComp){
				printf("Size reduction! pre: %lu, post: %lu\n", OSize_preComp, OSize_postComp);
			}
			llb->AddToLLB('O', Req::read, OSize_postComp-OSize_preComp, DONT_UPDATE_TRAFFIC);
			...
			o_r_row_acc += OSize_preComp;
			// This is read, so we can bring data earlier!
			/*excessCycles =*/ updateBWLogAndReturnExcess(batch_starting_cycle, batch_ending_cycle,
	...
	if(o_r_row_acc  != o_r_row_calc){
		printf("1: i_idx:%d, j_idx: %d-%d, k_idx: [%d-%d]\n", i_idx, j_idx_start, j_idx_stop, k_idx, std::min(k_idx+a_reuse, o_tiled_cols));
		printf("i_idx %d: There is a mismatch for output row read size! Acc in Schedule: %lu, Calc beforehand: %lu\n", i_idx, o_r_row_acc, o_r_row_calc);
		...
		for(int k_inner = k_idx; k_inner < std::min(k_idx+a_reuse, o_tiled_cols); k_inner++){
			for(int t_idx = 0; t_idx<a_entries_count.size();t_idx++){
				...
				intersectTwoVectors(vecA_begin, vecA_end, vecB_begin, vecB_end,
						...
						a_idx, b_idx, len_B_fiber);
				if(intersect_vector.size() >0){
					printf("i_idx: %d, k_idx: %d, lower_bound: %d, upper_bound: %d\n", i_idx, k_inner, lower_bound, upper_bound);
					int * p1 = vecA_begin; int * p2 = vecB_begin;
					printf("Vector A:");
					while((p1 <= vecA_end) & (*p1<upper_bound)){
						printf(" %d", *p1);
						p1++;
					}
					printf("\nVector B:");
					while((p2 <= vecB_end) & (*p2<upper_bound)){
						printf(" %d", *p2);
						p2++;
					}
					printf("\nIntersect Vector (%lu):", intersect_vector.size());
					for(int i=0;i<intersect_vector.size();i++)
						printf(" %d ", intersect_vector[i]);
					printf("\n-----------------------\n");
				}
			}
		}
...
void Scheduler_7::findJIndexesInTheRange(int i_idx, int lower_bound, int upper_bound,
		int * vecA_begin, int * vecA_end,
		std::vector<int> & j_indexes, std::vector<int> & a_entries_count){

	// The case for instant is so unrealistic! Since it assumes that all
	// basic tiles will fit in one PE buffer.
	// Anyway, it should fit into the middle DOT buffer
	//	by commenting the next lines, it should work fine

	// IF we have instant model for the middle level nothing is needed
	// When middle DOT is enabled, this becomes important
/*
...
*/
	...
	for(int * it = vecA_begin; it<=vecA_end; it++){
		...
		total_tile_count++;
		// Next tile size
		int tile_size = matrix->getCSFSize('A', i_idx, j_idx);
...
uint64_t Scheduler_7::updateBWLogAndReturnExcessMiddleDOT(uint64_t starting_cycle,
...
uint64_t Scheduler_7::updateBWLogAndReturnExcess(uint64_t starting_cycle,
		uint64_t ending_cycle, uint64_t action_bytes,uint64_t & finish_cycle ){
...
//		the fetched row
void Scheduler_7::SyncPETimesWithMinCycle(uint64_t min_val){
...
void Scheduler_7::CalcOReuse(int k_idx, int j_idx_start, int & j_idx_stop){
	...
	if(params->getTilingMechanism() == tiling::t_static){
		...
		uint64_t extra_size = AccumulateSize('B', j_idx_start, j_idx_stop,
				k_idx, std::min(k_idx+K_top_tile, o_tiled_cols));
...
// Find out what is the A and O row size we need to fetch
//	after LLB intersection
void Scheduler_7::PreCalculateAORowsSize(int j_start, int j_end,
...
void Scheduler_7::FetchAORows(int & numRows,
	...
	while(num_vecCommittedRows>0){
		...
		if((a_row_size + o_row_size) != 0){
			...
			llb->RemoveFromLLB('A', Req::write, a_row_size, DONT_UPDATE_TRAFFIC);
	...
	if(params->getTilingMechanism() == tiling::t_static){
		...
		if(remaining_row_slots > 0){
			// Find the last fetchable idx based on number of available row slots
...
uint64_t Scheduler_7::findFinishTimeTopLevel(uint64_t vec_A_time,
...
void Scheduler_7::CalcBLLBHorizontalSum(int j_start, int j_end,
 ...
 */
uint64_t Scheduler_7::AccumulateSize_AwrtB(int i_idx,
...
//  looks at the A&B intersection in the LLB to see whether
//  they should be loaded or not
uint64_t Scheduler_7::AccumulateSize_OwrtAB(int i_idx,
		int j_start, int j_end, int k_start, int k_end){

	uint64_t non_filtered_size = AccumulateSize('O',i_idx, i_idx+1, k_start, k_end);

	uint64_t size = 0;
	for(int k_idx = k_start; k_idx<k_end; k_idx++){
		// Looks into the intersection of the specifi A row and B column
		// In case the intersection is not empty, fetches in the output tile
...
uint32_t Scheduler_7::ShouldIFetchThisOTile(int i_idx,
	...
	if (j_idx_start == 0)
		return 0;
	int lower_bound = j_idx_start; int upper_bound = j_idx_end;
	...
	match = intersectTwoVectors(vecA_begin, vecA_end,
		vecB_begin, vecB_end,
...
//   empty: 0
//   not empty: 1
int Scheduler_7::getOverheadIntersectTwoVectors(int * vec1_begin, int * vec1_end,
...
//   empty: 0
//   not empty: 1
int Scheduler_7::intersectTwoVectors(int * vec1_begin, int * vec1_end,
		int * vec2_begin,	int * vec2_end, int lower_bound, int upper_bound){
...
//   empty: 0
//   not empty: 1
void Scheduler_7::intersectTwoVectors(int * vec1_begin, int * vec1_end,
		int * vec2_begin,	int * vec2_end, int lower_bound, int upper_bound,
...
/*
 // DEBUGGING: Results are correct
	p1 = vec1_begin; p2 = vec2_begin;
	printf("Vector A:");
	while((p1 <= vec1_end) & (*p1<upper_bound)){
		printf(" %d", *p1);
		p1++;
	...
	printf("\nVector B:");
	while((p2 <= vec2_end) & (*p2<upper_bound)){
		printf(" %d", *p2);
		p2++;
	}
	printf("\nIntersect Vector (%d):", intersect_vector.size());
	for(int i=0;i<intersect_vector.size();i++)
		printf(" %d", intersect_vector[i]);
	printf("\n-----------------------\n");
*/
...
void Scheduler_7::CalculateMiddleLevelExtarctOverhead(int a_vec_count, int b_vec_count,
...
//   for extracting the A row and corresponding B columns
//   (only entries between lower_bound and upper_bound for both vec A and vec B)
void Scheduler_7::CalculateMiddleLevelIntersectOverhead(
...
uint64_t Scheduler_7::AccumulateSize(char mat_name, int d1_start, int d1_end,
...
uint64_t Scheduler_7::AccumulateNNZ(char mat_name, int d1_start, int d1_end,
	 	int d2_start, int d2_end){
...
uint64_t Scheduler_7::AccumulateNNR(char mat_name, int d1_start, int d1_end,
	 	int d2_start, int d2_end){
...
uint64_t Scheduler_7::AccumulateNNZTiles(char mat_name, int d1_start, int d1_end,
	 	int d2_start, int d2_end){
