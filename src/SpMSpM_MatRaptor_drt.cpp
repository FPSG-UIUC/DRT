...
int main(int argc, char *argv[])
{
	...
	printf("Toluwa DEBUG: setting up matrix\n");
	fflush(stdout);
        //The Asquared version
	Matrix * mat = new Matrix(ip->p_input1, params, stats, llb);
	printf("Toluwa DEBUG: Done setting up matrix\n");
	fflush(stdout);
	Scheduler_10_drt * sched_10_drt = new Scheduler_10_drt(mat, params, stats, llb);
	sched_10_drt->Run();
...
void initializeInputParameters(input_parameters_st *input_params){
	...
	input_params->p_b_format = CSX::CSF; input_params->p_o_format = CSX::COO; input_params->p_dense_cols=1;
	...
	input_params->p_a_llb_perc = 0.5;	input_params->p_b_llb_perc = 0.05;	input_params->p_o_llb_perc = 0.45;
	// LLB Size
