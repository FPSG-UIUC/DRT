...
int Scheduler_8::Run(){
	...
	for(int k_start_top = 0; k_start_top < o_tiled_cols; /* EMPTY */){
		...
		while(j_start_top < b_tiled_rows){
            ...
            ExtractBTopTile(k_start_top, j_start_top, end_top);
            ...
            //Needs to be updated by the ExtractBTopTile function
            k_end_top = end_top[Dim::k_dim];
            j_end_top = end_top[Dim::j_dim];

			printf("k_end_top, j_end_top: %d %d\n", k_end_top, j_end_top);
			fflush(stdout);
...
//A: IxJ B: JxK O: IxK
//We're growing diagonally where possible.
void Scheduler_8::ExtractBTopTile(int k_idx_start, int j_idx_start, std::map<Dim, int> &end_top){

	int a_reuse = params->getAReuse();

    //{}_idx_start and {}_idx_stop variables will be tracked for the total macrotile size
    //local {}_idx variables will track individual iterative growths
	int j_idx_stop = end_top[Dim::j_dim];
	int k_idx_stop = end_top[Dim::k_dim];//std::min(k_idx+a_reuse, o_tiled_cols);


	printf("In ExtractBTopTile %d %d\n", j_idx_stop, k_idx_stop);
	fflush(stdout);
	
	// In static tiling case we already now what the j_idx_end
	//	should be and have set the OReuse value in params
	if(params->getTilingMechanism() == tiling::t_static){
		j_idx_stop = std::min(j_idx_start + params->getOReuse(),
				b_tiled_rows);

	    k_idx_stop = std::min(k_idx_start+a_reuse, o_tiled_cols);
        //Why is this j_idx_stop??? - doesn't matter, 
        // since we don't refer to the 2nd j_idx_stop in the
        // accumulatesize function
		uint64_t extra_size = AccumulateSize('B', j_idx_start, j_idx_stop,
				k_idx_start, j_idx_stop, params->getBFormat());
		llb->AddToLLB('B', Req::read, extra_size, UPDATE_TRAFFIC);
	}
	else if(params->getTilingMechanism() == tiling::t_dynamic){
        //We need to track the size
        uint64_t buf_size = 0; //total size we've accumulated
        uint64_t extra_size = 0; //size on this growth iteration

        int j_idx = j_idx_start;
        int k_idx = k_idx_start; 

        /**********Scenario 1**********/
        // We're starting a fresh column (we haven't
        // grown the contracted dimension yet)
        if (j_idx_start == 0) {
            bool is_js_turn = true;
            //bool is_ks_turn = false;

            //Grow until we fill the buffer or we hit the dim limits
            while ( j_idx_stop < b_tiled_rows
                    && k_idx_stop < o_tiled_cols ) {
                if (is_js_turn) { //grow contracted by 1 (but really it's just one basic tile)
                    extra_size = AccumulateSize('B', j_idx, j_idx+1,
				                 k_idx_start, k_idx_stop, params->getBFormat());

                    	//Update our global stopping point for the contracted dim
                    j_idx_stop++;

                    //let's update j_idx to the next thing we'll want to grow
                    j_idx++;

                    buf_size += extra_size;
                    is_js_turn = false;

                    if (llb->DoesFitInLLB('B', extra_size) != 0) {
                        llb->AddToLLB('B', Req::read, extra_size, UPDATE_TRAFFIC);        
                    } else {
                        break;
                    }

                } //grew contracted by 1
                else { //it's k's turn (uncontracted) to grow by 1
                    extra_size = AccumulateSize('B', j_idx_start, j_idx_stop,
				                 k_idx, k_idx+1, params->getBFormat());
                    //printf("ks turn --> j: %d, %d, k: %d %d, %ld\n", 
                    //        j_idx_start, j_idx_stop, k_idx, k_idx_stop+1, extra_size);


                    //update our global stopping point
                    k_idx_stop++;
                ...
                } //grew uncontracted by 1
            } // finished growing diagonally (end while)
            ...
            //We're out of the while loop!
            //Which growth caused us to overflow? Decrement that by 1
            if (is_js_turn && (llb->DoesFitInLLB('B', buf_size) == 0) ) { //k caused the overflow
                k_idx_stop--; //decrement k
				k_idx--;
                buf_size -= extra_size;
            ...
            else if (!is_js_turn && (llb->DoesFitInLLB('B', buf_size) == 0) ) { //j caused the overflow
                j_idx_stop--; //decrement j
				j_idx--;
                buf_size -= extra_size;
            ...
            //Or perhaps we just reached the end of a dimension. Try growing along the other one
            if ( grow_j || (k_idx_stop == o_tiled_cols) ) {
                ...
                while (j_idx_stop < b_tiled_rows) {
                    ...
                    if (llb->DoesFitInLLB('B', buf_size) == 0) { //check if it fits
                        buf_size -= extra_size;
                        j_idx_stop--;
						j_idx--;
                        break; //stop growing!
                ...
                } //we've reached the limit of j
				printf("Left the 2nd while loop\n");
				fflush(stdout);

            } 
            else if (grow_k || (j_idx_stop == b_tiled_rows) ) {
            
                while (k_idx_stop < o_tiled_cols) {
                    extra_size = AccumulateSize('B', j_idx_start, j_idx_stop,
                                 k_idx, k_idx++, params->getBFormat());
                ...
                } //we've reached the limit of k
				printf("Left the 3rd while loop\n");
				fflush(stdout);
            }


            /**********Sanity Check**********/
            if (k_idx_stop > o_tiled_cols) {
                printf("Error! k_idx_stop is past where it should be!\n");
                exit(1);
            } 

            if (j_idx_stop > b_tiled_rows) {
                printf("Error! j_idx_stop is past where it should be!\n");
                exit(1);
            }
            /*********End Sanity Check********/
        } 
        ...
        /**********Scenario 2**********/
        else { //we already grew the uncontracted, so we can only grow the contracted dim
            //Grow J 
            while (j_idx_stop < b_tiled_rows) {
                extra_size = AccumulateSize('B', j_idx, j_idx+1,
                                 k_idx_start, k_idx_stop, params->getBFormat());
                j_idx_stop = j_idx_stop + 1;
                j_idx = j_idx + 1;
			...
			printf("Left the 4th while loop\n");
			fflush(stdout);
        }
        /**********End Scenario 2**********/
		

        //We now have the overall macro tile size.
        //Officially add it to the LLB - this may cause overflow though...
        //fixed by incrementally adding above.
        //llb->AddToLLB('B', Req::read, buf_size, UPDATE_TRAFFIC);        
	}
	else{
		printf("No Such Tiling Mechanism is Available!\n");
	}
	printf("Grew the tile\n");
	fflush(stdout);
	...
	// Parallel and serial cases
		...
		for(int idx = k_idx_start; idx< k_idx_start+length_array; idx++){
			...
			per_col_runtime[idx-k_idx_start] = nnz+nnr;
	...
	if(params->getSearchModelTop() == search_tiles::instant)
		...
		for(int idx = k_idx_start; idx< k_idx_stop; idx++){
	...
	*/
    ...
    end_top[Dim::k_dim] = k_idx_stop;
    end_top[Dim::j_dim] = j_idx_stop;
    printf("We grew j (%d, %d) and k (%d, %d)\n", 
            j_idx_start, j_idx_stop, k_idx_start, k_idx_stop);
	fflush(stdout);
}
