...
OUTDIR = os.getcwd() + "/results_constreuse_sweep_asquared"
...
if not os.path.exists(EXECDIR + "/SpMSpM_TACTile_diag_drt"):
    print("Error: SpMSpM_TACTile_diag_drt binary not found", file=sys.stderr)
    exit(1)
    ...
    for constreuse in [128]:
        ...
        for dataset in matrix_paths:
            if dataset.endswith(".mtx"):
                ...
                cmd   = EXECDIR + "/SpMSpM_TACTile_diag_drt "
                ...
                test_folder = TEST_LOG_DIR + "/res_tactile_asquared_diag_"+graph+"_cfi_rr_"+\
                                             abo_string + "_diag" + str(constreuse)
                cmd += " > " + test_folder + "/res_tactile_asquared_diag_"+graph+"_cfi_rr_"+\
                                             abo_string + "_diag" + str(constreuse) + ".txt;"
                #print(cmd)
