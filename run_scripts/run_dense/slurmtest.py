...
OUTDIR = os.getcwd() + "/results_dense"
        ...
        for k in [32, 48]:#[24,32,48]:
            for dataset in os.listdir(DATADIR):
                if dataset.endswith(".mtx"):
                    ...
                    cmd = EXECDIR + "/SpMSpM_TACTile_twoInp_coo_output "
                    ...
                    test_folder = TEST_LOG_DIR + "/res_tactile_suc_dense_"+graph+"_cfi_rr_i"+str(i)+"_j"+str(j)+"_k"+str(k)
                    cmd += " > " + test_folder + "/res_tactile_suc_dense_"+graph+"_cfi_rr_i"+str(i)+"_j"+str(j)+"_k"+str(k)+".txt;"
                    ...
                    if (i==32 and j==32 and k==32):
                        ...
                        cmd +=  "--inp1="+DATADIR+"/"+graph+".mtx "
                        cmd +=  "--inp2="+DATADIR+"/"+graph+".mtx "
                        cmd +=  "--tiledim=32 --staticdist=rr --intersect=parbi --aperc=0.01 --bperc=0.3 --operc=0.69 --llbpartition=const"
                        test_folder = TEST_LOG_DIR + "/res_tactile_drt_dense_"+graph+"_transpose_cfi_rr_i"+str(i)+"_j"+str(j)+"_k"+str(k)
                        cmd += " > " + test_folder + "/res_tactile_drt_dense_"+graph+"_transpose_cfi_rr_i"+str(i)+"_j"+str(j)+"_k"+str(k)+".txt;"
                        subprocess.run("mkdir -p " + test_folder, shell=True)
                        gpu_args.append([test_folder, cmd])

                    #extensor
                    cmd = EXECDIR + "/SpMSpM_ExTensor "
                    cmd +=  "--inp1="+DATADIR+"/"+graph+".mtx "
                    cmd +=  "--inp2="+DATADIR+"/"+graph+".mtx "
                    cmd +=  "--tiledim=32 --staticdist=rr --intersect=skip "
                    cmd +=  "--itop="+str(i)+" --jtop="+str(j)+" --ktop="+str(k)

                    test_folder = TEST_LOG_DIR + "/res_tactile_extensor_dense_"+graph+"_transpose_skip_rr_i"+str(i)+"_j"+str(j)+"_k"+str(k)
                    cmd += " > " + test_folder + "/res_tactile_extensor_dense_"+graph+"_transpose_skip_rr_i"+str(i)+"_j"+str(j)+"_k"+str(k)+".txt;"
