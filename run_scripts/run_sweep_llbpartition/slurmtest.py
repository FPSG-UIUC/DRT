...
OUTDIR = os.getcwd() + "/results_llbpartitions_sweep_asquared"
...
percent_list =  [[0.33,0.33,0.33], 
                  [0.49,0.49,0.01], 
                  [0.45,0.05,0.50], 
                  [0.05,0.45,0.50], 

                  [0.01,0.30,0.69], #fix A, grow B (shrink O)
                  [0.01,0.50,0.49],
                  [0.01,0.60,0.39],
                  [0.01,0.80,0.19],
                  [0.01,0.90,0.09],
                  
                  [0.40,0.40,0.20], #grow O, keep A and B equal
                  [0.25,0.25,0.50], 
                  [0.10,0.10,0.80],
                  [0.05,0.05,0.90],
                  
                  
                  [0.30,0.25,0.45], #grow A, fix B, (shrink O)
                  [0.50,0.25,0.25],
                  [0.70,0.25,0.05],
                  [0.69,0.01,0.30]
                ]

percent_list2 = [ [0.20,0.75,0.05],
                  [0.15,0.60,0.05],
                  [0.25,0.70,0.05],
                  [0.30,0.65,0.05],
                  [0.35,0.60,0.05],
...
for abo_perc in  percent_list2:
    print("We have ", abo_perc)
    ...
    for dataset in matrix_paths:
        if dataset.endswith(".mtx"):
            ...
            test_folder = TEST_LOG_DIR + "/res_tactile_asquared_llbperc_"+graph+"_cfi_rr_"+\
                                         abo_string + llb_string
            cmd += " > " + test_folder + "/res_tactile_asquared_llbperc_"+graph+"_cfi_rr_"+\
                                         abo_string + llb_string + ".txt;"
            #print(cmd)
