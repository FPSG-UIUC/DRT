#!/usr/bin/env python3
# General slurm script, modified from Jonathan Wapman's original script
# LLB Sweeps for TACTile sweep
...
OUTDIR = os.getcwd() + "/results_llb_sweep_asquared"
...
for abo_perc in [ [0.01, 0.3, 0.69], [0.05, 0.45, 0.5] ]:
    for llbsize in [30, 2, 4, 8, 16, 32, 64]:
        ...
        for dataset in matrix_paths:
            if dataset.endswith(".mtx"):
                ...
                test_folder = TEST_LOG_DIR + "/res_tactile_asquared_llb_sweep_"+graph+"_cfi_rr_"+\
                                             abo_string + "_" + llb_string
                cmd += " > " + test_folder + "/res_tactile_asquared_llb_sweep_"+graph+"_cfi_rr_"+\
                                             abo_string + "_" + llb_string + ".txt;"
                #print(cmd)
